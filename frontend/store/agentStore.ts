...
interface AgentState {
  ...
  updateLastAriaMessage:  (content: string) => void;
...
export const useAgentStore = create<AgentState>()(
  devtools((set) => ({
    ...
    addMessage: (msg) =>
      set((s) => ({ messages: [...s.messages, { ...msg, id: uid(), timestamp: new Date() }] }), false, "addMessage"),

    // Finds the last "aria" bubble and updates its content — used for SSE token streaming
    updateLastAriaMessage: (content) =>
      set((s) => {
        const msgs = [...s.messages];
        for (let i = msgs.length - 1; i >= 0; i--) {
          if (msgs[i].role === "aria") { msgs[i] = { ...msgs[i], content }; break; }
        }
        return { messages: msgs };
      }, false, "updateLastAriaMessage"),

    setSessionId:         (id)     => set({ sessionId: id },           false, "setSessionId"),
    setLeadScore:         (score)  => set({ leadScore: score },         false, "setLeadScore"),
    setQualificationData: (data)   => set({ qualificationData: data },  false, "setQualificationData"),
    setStatus:            (status) => set({ status },                   false, "setStatus"),

    addToHistory: (role, content) =>
      set((s) => ({ conversationHistory: [...s.conversationHistory, { role, content }] }), false, "addToHistory"),
...
// Selector hooks — each subscribes only to its slice → minimal re-renders
